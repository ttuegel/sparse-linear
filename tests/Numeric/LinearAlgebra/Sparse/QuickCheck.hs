...
matchDims2 :: (FormatR fmt, Orient or, Orient or', Unbox a) => (Matrix fmt or a, Matrix fmt or' a) -> (Matrix fmt or a, Matrix fmt or' a)
matchDims2 (a, b) =
    let dima = a ^. dim
        dimb = b ^. dim
    in (a & dim .~ min dima dimb, b & dim .~ min dima dimb)

matchDims3 :: (FormatR fmt, Orient or, Orient or', Orient or'', Unbox a) => (Matrix fmt or a, Matrix fmt or' a, Matrix fmt or'' a) -> (Matrix fmt or a, Matrix fmt or' a, Matrix fmt or'' a)
matchDims3 (a, b, c) =
    let dim' = minimum [a ^. dim, b ^. dim, c ^. dim]
    in (a & dim .~ dim', b & dim .~ dim', c & dim .~ dim')

matchDimsV :: (FormatR fmt, Orient or, Unbox a) => (Matrix fmt or a, Vector a) -> (Matrix fmt or a, Vector a)
matchDimsV (m, v) =
    let c = minimum [U.length v, m ^. dim . _2]
    in (m & dim . _2 .~ c, U.take c v)

matchDimsV2 :: (FormatR fmt, Orient or, Orient or', Unbox a) => (Matrix fmt or a, Matrix fmt or' a, Vector a) -> (Matrix fmt or a, Matrix fmt or' a, Vector a)
matchDimsV2 (m, n, v) =
