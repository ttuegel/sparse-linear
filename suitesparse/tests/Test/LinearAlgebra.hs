...
prop_pointersNondecreasing :: Matrix Vector a -> Bool
prop_pointersNondecreasing Matrix{..} = nondecreasing pointers

prop_pointersLength :: Matrix Vector a -> Bool
prop_pointersLength Matrix{..} = V.length pointers == ncols + 1

prop_valuesLength :: Unbox a => Matrix Vector a -> Bool
prop_valuesLength Matrix{..} =
  V.length values == fromIntegral (V.last pointers)

prop_indicesIncreasing :: Unbox a => Matrix Vector a -> Bool
prop_indicesIncreasing mat =
  all (increasing . S.indices) $ map (slice mat) [0..(ncols mat - 1)]

prop_indicesNonNegative :: Unbox a => Matrix Vector a -> Bool
prop_indicesNonNegative = V.all (>= 0) . indices

prop_indicesInRange :: Unbox a => Matrix Vector a -> Bool
prop_indicesInRange Matrix{..} = V.all (< nrows) indices

checkFunMat1
  :: (Arbitrary a, Num a, Show a, Unbox a)
  => (Matrix Vector a -> Matrix Vector a) -> SpecWith ()
checkFunMat1 f = do
...
checkFunMat2
  :: (Arbitrary a, Num a, Show a, Unbox a)
  => (Matrix Vector a -> Matrix Vector a -> Matrix Vector a) -> SpecWith ()
checkFunMat2 f = do
